...
class RequireToSPLForm:
    ...
    async def require_to_splForm(self, db: AsyncSession):
        personas, audiences, terminologies, instructions = await self.conv_per_aud_des(self.prompt_conv_per_aud_des, self.user_description)
        splform = {'formData': []}
        personaData = {
        ...
        context_rules = await self.context_control(self.prompt_context_control, self.user_description, personas, audiences, instructions)
        contextRuleData ={
            "sectionId": str(len(splform['formData'])),
