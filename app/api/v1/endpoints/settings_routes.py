...
async def create_settings_endpoint(settings_data: SettingsCreate, db: AsyncSession = Depends(get_db_session), _: None = Depends(get_current_user)):
...
async def edit_settings_endpoint(settings_id: int, update_data: SettingsUpdate, db: AsyncSession = Depends(get_db_session), _: None = Depends(get_current_user)):
...
async def get_settings_by_id_endpoint(settings_id: int, db: AsyncSession = Depends(get_db_session), _: None = Depends(get_current_user)):
...
async def delete_settings_endpoint(settings_id: int, db: AsyncSession = Depends(get_db_session), _: None = Depends(get_current_user)):
...
async def get_all_settings_endpoint(db: AsyncSession = Depends(get_db_session), _: None = Depends(get_current_user)):
